...
import AdminCategorys from "src/views/admin/item/detail/AdminCategorys";
import AdminItemOptions from "src/views/admin/item/detail/ItemOptions";
...
const ItemForm = () => {
  ...
  const saveItem = (e) => {
    ...
    for(const inp of sizes) {
      if(!inp.value.trim()) {
        alert('사이즈 정보를 입력해주세요')
        return
      }
    }
  ...
  useEffect(() => {
    if(itemGroup.id) {
      getCategory(itemGroup.category.id)
    }
  }, [depth1]);
  ...
  const getItemGroup = () => {
    axios.get(`http://localhost:3011/item/test/${itemGroupId}`).then((res) => {
      ...
      setIsDiscount(res.data.isDiscounted == 1 ? true : false)
      setDiscountRate((((res.data.defaultPrice - res.data.salePrice) / res.data.defaultPrice) * 100).toFixed(2) + '%')
      setRealPrice(numberComma(res.data.salePrice))
      setId(res.data.id)
      let arr = []
  ...
  const getParent = () => {
    axios.get(`http://localhost:3011/admin/category/parent/${itemGroup.category.id}`).then((res) => {
      document.querySelector('.depth_1').value = res.data.id
      getCategory(res.data.id)
    })
  }
  ...
  useEffect(() => {
    if(depth2.length) {
      document.querySelector('.depth_2').value = itemGroup.category.id
    }
  }, [depth2]);
            ...
            <CCardBody>
              ...
              <CRow>
                <CCol xs={6}>
                  ...
                  <CFormSelect className={'depth_1'} onChange={selectDepth1}>
                    <option value="0">1차 카테고리</option>
                    <AdminCategorys categorys={depth1} itemCategory={categoryInfo} depth={1} />
                  </CFormSelect>
                </CCol>
                <CCol xs={6}>
                  <p className="text-body-secondary small">
                  ...
                  <CFormSelect className={'depth_2'} name={'categoryId'}>
                    <option value='0'>2차 카테고리</option>
                    <AdminCategorys categorys={depth2} itemCategory={categoryInfo} depth={2} />
                  </CFormSelect>
                </CCol>
              </CRow>
              <br/>
              <CRow>
                ...
                <CCol xs={1}>
                  <CFormCheck
                    ...
                    name="gender"
                    label="공용"
                    defaultChecked={itemGroup.gender == 'all'}
                    value={'all'}
                  />
                </CCol>
                <CCol xs={1}>
                  <CFormCheck
                    ...
                    name="gender"
                    label="남"
                    defaultChecked={itemGroup.gender == '남'}
                    value={'M'}
                  />
                </CCol>
                <CCol xs={1}>
                  <CFormCheck
                    ...
                    name="gender"
                    label="여"
                    defaultChecked={itemGroup.gender == '여'}
                    value={'w'}
                  />
                </CCol>
              </CRow>
            ...
            <CCardBody>
              <CRow>
                ...
                <CCol xs={1}>
                  <CFormCheck
                    ...
                    label="노출"
                    value={1}
                    defaultChecked={itemGroup.isView == 1 ? true : false}
                  />
                </CCol>
                <CCol xs={2}>
                  <CFormCheck
                    ...
                    label="미노출"
                    value={0}
                    defaultChecked={itemGroup.isView == 0 ? true : false}
                  />
                </CCol>
              </CRow>
              <br/>
                ...
                <CCol xs={6}>
                  <CInputGroup className="mb-3">
                    ...
                    <CFormInput
                      ...
                      defaultValue={itemGroup.defaultPrice > 0 ? numberComma(itemGroup.defaultPrice) : ''}
                      required
                    />
                  </CInputGroup>
                  <CFormCheck
              ...
              <CTable hover>
                ...
                <CTableBody>
                  {
                    sort.map((st, index) => (
                      <AdminItemOptions num={st} key={index} data={itemGroup.items ? (itemGroup.items)[index] : {}}/>
                    ))
                  }
                </CTableBody>
              </CTable>
        ...
        <CCol xs={12}>
          <CCard className="mb-4">
            ...
            <CCardBody className={'sizeInfo'}>
              <SizeTable data={itemGroup.sizeTable} />
            </CCardBody>
          </CCard>
        </CCol>
        <CRow>
