...
function UnregisterList() {
  ...
  const getCurrentPageList = () => {
    ...
    return unregisterList.slice(startIndex, endIndex);
  }
  ...
  const handleSearch = async () => {
    try {
      ...
      const response = await AdminMemberService.searchAdminUnregister(searchType, keyword);
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          ...
          <CCardBody>
              ...
              <CTable style={{textAlign: 'center'}}>
                <CTableHead color="light">
                  <CTableRow>
                    ...
                    <CTableHeaderCell scope="col" style={{width: '20%'}}>탈퇴 이메일</CTableHeaderCell>
                ...
                <CTableBody>
                  {getCurrentPageList().map((unregister, index) => (
                    <CTableRow key={index}>
                      <CTableHeaderCell scope="row" style={{width: '10%'}}>{unregister.id}</CTableHeaderCell>
                      <CTableDataCell style={{width: '20%'}}>{unregister.email}</CTableDataCell>
                      <CTableDataCell style={{width: '20%'}}>{unregister.name}</CTableDataCell>
                      <CTableDataCell style={{width: '20%'}}>{unregister.nickname}</CTableDataCell>
                      <CTableDataCell style={{width: '10%'}}>{unregister.role}</CTableDataCell>
                      <CTableDataCell style={{width: '20%'}}>
                        <Link to={`/admin/member/unregisterDetail/${unregister.id}`}>
            ...
            {/* 페이지네이션 */}
            {unregisterList.length > itemsPerPage && (
              <CPagination align="center" aria-label="Page navigation example">
                ...
                {Array.from({length: Math.ceil(unregisterList.length / itemsPerPage)}, (_, i) => (
                  <CPaginationItem
                    key={i + 1}
                ...
                <CPaginationItem
                  aria-label="Next"
                  onClick={() => handlePageChange(currentPage + 1)}
                  disabled={currentPage === Math.ceil(unregisterList.length / itemsPerPage)}
                >
